...
| FR-006 — Non-disableable safety finalization | **Partial** | [TASK-016 verification](task-016-verification.md) | Motor 2.0 now has one versioned rule/source register: all ten rules have stable IDs, severity, traceable sources, an explicit non-override contract, and fail-closed NO/SE/DK review status. Safety is also reapplied after legacy overrides. The remaining gaps are localized message keys in rule output and the production Home path omitting the active child's `canRoll`, so a child under four months who already rolls may miss the legacy HB-6 swaddle removal until TASK-017. |
