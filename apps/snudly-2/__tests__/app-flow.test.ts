...
describe('Snudly 2 app flow', () => {
  ...
  it('routes returning users through tour, paywall or home from stored truth', () => {
    ...
    const paywall = createInitialFlow({ ...newUser, onboardingComplete: true, tourComplete: true, profile });
    const home = createInitialFlow({ ...newUser, onboardingComplete: true, tourComplete: true, entitlementActive: true, profile });

    expect(reduceAppFlow(tour, { type: 'release-launch' })).toMatchObject({ screen: 'tour', page: 'home', profile });
    expect(reduceAppFlow(paywall, { type: 'release-launch' })).toMatchObject({ screen: 'paywall', profile });
  ...
  it('moves from welcome through all four real product pages into hard paywall', () => {
    let state = reduceAppFlow(createInitialFlow(newUser), { type: 'release-launch' });
    state = reduceAppFlow(state, { type: 'next' });
    state = reduceAppFlow(state, { type: 'set-birth-date', value: '2025-10-17' });
    state = reduceAppFlow(state, { type: 'next' });
    ...
    state = reduceAppFlow(state, { type: 'replay-tour' });
    expect(state).toMatchObject({ screen: 'tour', page: 'home' });
  ...
  it('keeps the paywall hard until a verified entitlement is supplied at launch', () => {
    const paywall = reduceAppFlow(
      createInitialFlow({ ...newUser, onboardingComplete: true, tourComplete: true }),
      { type: 'release-launch' },
    );

    expect(paywall).toMatchObject({ screen: 'paywall' });
    expect(reduceAppFlow(paywall, { type: 'release-launch' })).toEqual(paywall);
    expect(reduceAppFlow(paywall, { type: 'start-tour' })).toEqual(paywall);
    expect(reduceAppFlow(paywall, { type: 'replay-tour' })).toMatchObject({
      screen: 'tour',
      page: 'home',
    });
  });
