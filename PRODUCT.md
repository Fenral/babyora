...
**Owner decisions 2026-07-31 (round 3):** (1) Mascot idle-loop video ships on the Hjem panel (paused under reduced motion; same pattern as the onboarding intro). (2) Activity toggle triggers automatic inline recalculation with the short scan choreography; the stale screen remains as fallback when recalculation is slow or fails — this supersedes the handoff's two-tap stale model. (3) v1 ships dark-only; the calibrated light mode follows in the first update. (4) Implementation P1-P9 authorized to run autonomously package by package with a green test suite as gate.
