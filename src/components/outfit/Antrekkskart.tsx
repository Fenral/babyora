...
export function Antrekkskart({ snapshot, selectedId, highlightedId, captionId, onActivate, onFocus, onHover }: Props) {
  const layout = layoutOutfitMap(snapshot, 560);
  ...
  return <section className={`outfit-map outfit-map--${layout.mode}`} aria-label="Antrekkskart" data-outfit-map-mode={layout.mode}>
