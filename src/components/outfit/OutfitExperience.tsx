...
type Props = Readonly<{ snapshot: OutfitTruthSnapshotV1; options?: readonly OutfitAlternativeOptionV1[]; blockedAlternatives?: readonly OutfitBlockedAlternativeV1[]; temp: 'kald' | 'mild' | 'varm'; registerOutfitRow?: RegisterOutfitRow; }>;
const EMPTY_OPTIONS = Object.freeze([]) as readonly OutfitAlternativeOptionV1[];
const EMPTY_BLOCKED_ALTERNATIVES = Object.freeze([]) as readonly OutfitBlockedAlternativeV1[];
...
export function OutfitExperience({ snapshot, options = EMPTY_OPTIONS, blockedAlternatives = EMPTY_BLOCKED_ALTERNATIVES, temp, registerOutfitRow }: Props) {
  ...
  return <section className="outfit-experience ba-temp-root" data-temp={temp}><Antrekkskart snapshot={current} selectedId={selectedId} highlightedId={highlightedId} captionId={captionId} onActivate={activate} onFocus={setFocusId} onHover={setHoverId} /><p ref={captionRef} tabIndex={-1} id={captionId} className="outfit-active-caption">{captionSource !== undefined ? displayNameForDbString(captionSource) : 'Antrekk'}</p><OutfitGarmentList snapshot={current} selectedId={selectedId} highlightedId={highlightedId} captionId={captionId} registerOutfitRow={registerOutfitRow} blockedAlternatives={blockedAlternatives} onActivate={activate} onFocus={setFocusId} onHover={setHoverId} hasAlternative={(id) => authorizedOptions.some((candidate) => candidate.sourceItemId === id)} onAlternative={(id, trigger) => { comparisonFocusLifecycle.open(trigger); setCompareId(id); }} />{current !== snapshot && <button type="button" onClick={() => { reset(); }}>Tilbakestill antrekk</button>}{option && <OutfitComparisonDialog option={option} sourceLabel={current.garments.find((item) => item.itemId === option.sourceItemId)?.label ?? ''} headingRef={comparisonHeadingRef} onConfirm={() => { if (select(option).ok) { setSelectedId(null); finishComparison(false); captionRef.current?.focus(); } }} onCancel={closeComparison} />}<div className="outfit-recovery-copy" /></section>;
}
